...
df = pd.read_csv('/Users/stomar-n/001_sudhir_2020_nmac/projects/cognixia/data/titanic.csv')
print(df.info())
...
def func(x):
    if pd.isnull(x[0]):
        ...
        return x[0]
print(df[['Age','Pclass']].apply(func,axis=1))

def funcClass(x):
    if pd.isnull(x[0]):
        return df[df['Pclass'] == x[1]]['Age'].mean()
    else:
        return x[0]
df.Age = df[['Age','Pclass']].apply(funcClass,axis=1)
print(df.info())
print(df[['Age','Pclass']].apply(funcClass,axis=1))
df.Sex = df.Sex.apply(lambda x:1 if x=='male' else 0)
print(df.info())
print(df['Sex'])
print(pd.get_dummies(df.Embarked,prefix="Embarked"))
print(pd.get_dummies(df.Embarked,prefix="Embarked",drop_first=True))
df1 = pd.get_dummies(df.Embarked,prefix="Embarked",drop_first=True)
df = pd.concat([df,df1],axis=1)
print(df)
df = pd.read_csv('/Users/stomar-n/001_sudhir_2020_nmac/projects/cognixia/data/churn_data.csv')
print(df.info())
